 ...
 * Based on the algorithm by Stefan Gustavson (public domain).
...
class SimplexNoise {
  ...
  noise2D(xin, yin) {
    ...
    if (t0 < 0) n0 = 0.0;
    else { t0 *= t0; n0 = t0 * t0 * this.dot2(grad3[gi0], x0, y0); }

    let t1 = 0.5 - x1 * x1 - y1 * y1;
    if (t1 < 0) n1 = 0.0;
    else { t1 *= t1; n1 = t1 * t1 * this.dot2(grad3[gi1], x1, y1); }

    let t2 = 0.5 - x2 * x2 - y2 * y2;
    if (t2 < 0) n2 = 0.0;
    else { t2 *= t2; n2 = t2 * t2 * this.dot2(grad3[gi2], x2, y2); }

    return 70.0 * (n0 + n1 + n2);
  }

  noise3D(xin, yin, zin) {
    const { F3, G3, perm, permMod12, grad3 } = this;
    let n0, n1, n2, n3;
